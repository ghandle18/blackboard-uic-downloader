...
def handle_arguments(debug=False) -> None:
    ...
    if args.site is None:
        ...
        possible_site = config_content.get('site', '')
        if not args.ignore_input:
            user_input = input("Enter Black Board Host Website [{} or 'c' to Search]: ".format(possible_site))
            # If user pressed enter without typing, use the config file site
            args.site = user_input if user_input.strip() else possible_site
    ...
    # if args.dump:
    #    pass
    # Ensure args.institute is always defined to prevent the AttributeError
    args.institute = None 
    
    if args.site:
        if args.site is None: # Safety check
            print("No Site Supplied!")
            sys.exit(0)
        try:
            # Try to fetch institute details based on the URL provided
            found_institutes = BlackBoardInstitute.find(args.site)
            if found_institutes:
                args.institute = found_institutes[0]
        except Exception:
            # If the lookup service is down, we leave it as None and 
            # let the BlackBoardClient handle it
            pass
