...
class BlackBoardClient:
    ...
    class LearnAPIVersion:
        ...
        def __init__(self, learn_version: str):
            ...
            # Helper to extract ONLY digits from a string
            def safe_parse(part):
                # This finds the first set of numbers and ignores the rest
                # e.g., "0-rel" becomes 0, "84" becomes 84
                match = re.search(r'\d+', part)
                return int(match.group()) if match else 0

            # Ensure we have at least 3 parts to avoid IndexError
            while len(self._raw) < 3:
                self._raw.append("0")
                
            self.major = safe_parse(self._raw[0])
            self.minor = safe_parse(self._raw[1])
            self.patch = safe_parse(self._raw[2])
        
        @staticmethod
        def _safe_int(version_part: str) -> int:
            """
            Extracts the leading digits from a version string part and converts to int.
            Example: '0-rel' -> 0, '123abc' -> 123
            """
            import re
            match = re.match(r'^\d+', version_part)
            return int(match.group()) if match else 0
...
def get_cookies(learn_url: str, default_browser: Optional[str] = None) -> CookieJar:
    ...
    Attempts to get a Users Login Session from a Browser
    """
    # FIX: Handle cases where learn_url might not have http/https prefix
    if "://" in learn_url:
        learn_domain = learn_url.split("://")[1].split("/")[0]
    else:
        learn_domain = learn_url.split("/")[0]
